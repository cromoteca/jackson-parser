...
public class SimpleConsoleOutput {
    public static void describe(ScanResult parserResult) {
        ...
        parserResult.endpoints().forEach(e -> {
            System.out.println(e.type().getName());
            e.methods().forEach(m -> System.out.format("%s(%s): %s\n", describe(m), parameters(m), describe(m.getAnnotatedReturnType())));
            System.out.println();
        });
        parserResult.entities().forEach(e -> {
    ...
    private static String describe(Member p) {
        ...
        var ann = other.distinct().map(a -> '@' + a.annotationType().getSimpleName()).sorted().collect(Collectors.joining(" "));
        return String.join(" ", ann, p.getName()).trim();
    }

    private static String parameters(Method m) {
        return Arrays.stream(m.getAnnotatedParameterTypes()).map(p -> String.format("%s: %s", p.getType(), describe(p))).collect(Collectors.joining(", "));
    }

    private static String describe(AnnotatedType at) {
        var ann = Arrays.stream(at.getAnnotations()).distinct().map(a -> '@' + a.annotationType().getSimpleName()).sorted().collect(Collectors.joining(" "));
    ...
    private static String describe(Type t) {
        if (t instanceof Class<?> c) {
            ...
            var y = Arrays.stream(pt.getActualTypeArguments()).map(SimpleConsoleOutput::describe).collect(Collectors.joining(", "));
            return x + "<" + y + ">";
